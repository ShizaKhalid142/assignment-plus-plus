  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto p-6">
        <h1 className="text-4xl font-bold text-white mb-8">📤 Student Submissions</h1>
        ...
        <div className="mb-6 bg-white/5 rounded-2xl border border-white/10 p-6 backdrop-blur-sm">
          <form onSubmit={search} className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="block text-sm font-semibold text-white mb-2">Filter by Assignment</label>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Submission List */}
          <div>
            <h2 className="text-xl font-bold text-white mb-4">Submissions ({items.length})</h2>
              ...
              {items.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold text-white">🎓 Student {item.student_id}</p>
                      <p className="text-xs text-white/50 mt-1">{new Date(item.created_at || '').toLocaleDateString()}</p>
                    </div>
                    {item.grade && <span className="bg-green-500/20 text-green-300 px-2 py-1 rounded text-xs font-semibold">✓ Graded</span>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {selectedSubmission ? (
            <div className="bg-white/5 rounded-2xl border border-white/10 p-6 backdrop-blur-sm h-fit sticky top-6">
              <h3 className="text-xl font-bold text-white mb-4">📄 Submission Detail</h3>
              
              <div className="bg-white/10 rounded-lg p-4 mb-4 border-l-2 border-blue-500 space-y-2">
                <p className="text-sm text-white/70"><strong>Student:</strong> #{selectedSubmission.student_id}</p>
                <p className="text-sm text-white/70"><strong>Assignment:</strong> #{selectedSubmission.assignment_id}</p>
                <p className="text-sm text-white/70"><strong>Submitted:</strong> {new Date(selectedSubmission.created_at || '').toLocaleString()}</p>
                {selectedSubmission.file_name && <p className="text-sm text-white/70"><strong>📎 File:</strong> {selectedSubmission.file_name}</p>}
              </div>

              <div className="bg-white/10 rounded-lg p-4 mb-4 max-h-48 overflow-y-auto">
                <p className="text-sm font-semibold text-white mb-2">Submission Content:</p>
                <p className="text-sm text-white/70">{selectedSubmission.content || 'No text content'}</p>
              </div>

              {selectedSubmission.grade ? (
                <div className="bg-green-500/10 rounded-lg p-4 mb-4 border-l-2 border-green-500">
                  <p className="text-sm text-white/60"><strong>✓ Graded:</strong></p>
                  <p className="text-2xl font-bold text-green-400 mt-1">{selectedSubmission.grade}%</p>
                </div>
              ) : (
              ...
              {selectedSubmission.plagiarism_score !== undefined ? (
                ...
                <button onClick={() => checkPlagiarism(selectedSubmission.id)} disabled={checkingPlagiarism} className="w-full bg-white/10 text-white py-2 rounded-lg font-semibold hover:bg-white/15 transition disabled:opacity-50">
            ...
            <div className="bg-white/5 rounded-2xl border border-white/10 p-8 text-center flex items-center justify-center h-64">
              <p className="text-white/50">Select a submission to view details</p>
            </div>
          )}
        </div>
      </div>
